...
def create_mock_action(action_id: str, requires_approval: bool = False, risk_level: str = "LOW"):
    ...
    action.policy.approval_requirement.name = "REQUIRED" if requires_approval else "NONE"
...
def test_executor_cannot_invent_actions(mock_planner, mock_validator, mock_executor, mock_audit, pipeline_engine):
...
@patch('ai_engineering_bootstrap.pipeline.engine.ExecutionPlanValidator')
@patch('ai_engineering_bootstrap.pipeline.engine.PlannerEngine')
def test_safety_gate_blocks_execution(mock_planner, mock_validator, mock_executor, mock_audit, pipeline_engine):
    """Any action denied by SafetyGate must never reach its handler."""
    mock_planner.return_value.generate_plan.return_value = create_mock_plan("plan-1", [create_mock_action("dangerous")])
...
@patch('ai_engineering_bootstrap.pipeline.engine.ExecutionPlanValidator')
@patch('ai_engineering_bootstrap.pipeline.engine.PlannerEngine')
def test_human_approval_cannot_be_bypassed(mock_planner, mock_validator, mock_executor, mock_audit, pipeline_engine, approval_provider):
    """If an action requires approval and it is missing, execution is blocked."""
    mock_planner.return_value.generate_plan.return_value = create_mock_plan("plan-1", [create_mock_action("act-1", requires_approval=True)])
    mock_validator.return_value.validate.return_value = MagicMock(is_valid=True)
...
@patch('ai_engineering_bootstrap.pipeline.engine.ExecutionPlanValidator')
@patch('ai_engineering_bootstrap.pipeline.engine.PlannerEngine')
def test_real_mode_remains_explicit(mock_planner, mock_validator, mock_executor, mock_audit, pipeline_engine):
    """Default execution mode must remain SAFE."""
    mock_planner.return_value.generate_plan.return_value = create_mock_plan("plan-1", [create_mock_action("act-1")])
    mock_validator.return_value.validate.return_value = MagicMock(is_valid=True)
...
@patch('ai_engineering_bootstrap.pipeline.engine.ExecutionPlanValidator')
@patch('ai_engineering_bootstrap.pipeline.engine.PlannerEngine')
def test_recovery_does_not_bypass_safety(mock_planner, mock_validator, mock_executor, mock_audit, pipeline_engine):
...
@patch('ai_engineering_bootstrap.pipeline.engine.ExecutionPlanValidator')
@patch('ai_engineering_bootstrap.pipeline.engine.PlannerEngine')
def test_unknown_actions_blocked(mock_planner, mock_validator, mock_executor, mock_audit, pipeline_engine):
    """Unknown action IDs must never execute."""
    mock_planner.return_value.generate_plan.return_value = create_mock_plan("plan-1", [create_mock_action("unknown_act")])
    mock_validator.return_value.validate.return_value = MagicMock(is_valid=False)
...
@patch('ai_engineering_bootstrap.pipeline.engine.ExecutionPlanValidator')
@patch('ai_engineering_bootstrap.pipeline.engine.PlannerEngine')
def test_replan_is_observable(mock_planner, mock_validator, mock_executor, mock_audit, pipeline_engine):
...
@patch('ai_engineering_bootstrap.pipeline.engine.ExecutionPlanValidator')
@patch('ai_engineering_bootstrap.pipeline.engine.PlannerEngine')
def test_malicious_action_ids_rejected(mock_planner, mock_validator, mock_executor, mock_audit, pipeline_engine, malicious_id):
    """Malicious or malformed inputs must never be executed."""
    mock_planner.return_value.generate_plan.return_value = create_mock_plan("plan-1", [create_mock_action(malicious_id)])
    mock_validator.return_value.validate.return_value = MagicMock(is_valid=False)
...
@patch('ai_engineering_bootstrap.pipeline.engine.ExecutionPlanValidator')
@patch('ai_engineering_bootstrap.pipeline.engine.PlannerEngine')
def test_e2e_healthy_flow(mock_planner, mock_validator, mock_executor, mock_audit, pipeline_engine):
    """Scenario 1: Healthy environment completes deterministically."""
    mock_planner.return_value.generate_plan.return_value = create_mock_plan("plan-1", [create_mock_action("act-1")])
    mock_validator.return_value.validate.return_value = MagicMock(is_valid=True)
    mock_executor.return_value.execute.return_value = MagicMock(results=[MagicMock(is_success=True)], is_success=True)
...
@patch('ai_engineering_bootstrap.pipeline.engine.ExecutionPlanValidator')
@patch('ai_engineering_bootstrap.pipeline.engine.PlannerEngine')
def test_e2e_approved_action_proceeds(mock_planner, mock_validator, mock_executor, mock_audit, pipeline_engine, approval_provider):
