...
def test_package_actions_have_unique_ids_and_independent_approval() -> None:
    service = RuntimeSessionService(repository=InMemorySessionRepository(), audit_factory=_audit_factory)
...
def test_real_start_executes_only_approved_action_instances() -> None:
    ...
    class FakeBootstrap:
        def run(self, **kwargs):
            ...
            return SimpleNamespace(is_success=True, environment_ready=True, rejected_actions=[], action_results=[])
    ...
    result = service.start(session_id, mode=ExecutionMode.REAL)
