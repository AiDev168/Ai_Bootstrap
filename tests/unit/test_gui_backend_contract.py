...
def backend_with_temp_settings(monkeypatch, tmp_path) -> ApplicationBackend:
    ...
    for name in (LLMSettingsService.ENV_PROVIDER, LLMSettingsService.ENV_MODEL, LLMSettingsService.ENV_BASE_URL, LLMSettingsService.ENV_API_KEY):
        monkeypatch.delenv(name, raising=False)
    session_service = EnvironmentSessionService(
        repository=InMemorySessionRepository(),
        audit_factory=lambda: SimpleNamespace(checks=[]),
    )
    return ApplicationBackend(session_service=session_service)


def test_session_endpoints_return_serializable_data(monkeypatch, tmp_path) -> None:
...
def test_llm_settings_round_trip_is_secret_safe(monkeypatch, tmp_path) -> None:
    backend = backend_with_temp_settings(monkeypatch, tmp_path)
    result = backend.update_llm_settings({"provider": "remote_api", "model": "test-model", "base_url": "https://example.invalid/v1", "api_key": "secret-value"})
...
def test_unsaved_llm_probe_does_not_change_persisted_settings(monkeypatch, tmp_path) -> None:
    backend = backend_with_temp_settings(monkeypatch, tmp_path)
    backend.update_llm_settings({"provider": "mock", "model": "saved-model", "base_url": "", "api_key": ""})
    probe = backend.test_llm_connection({"provider": "local_server", "model": "unsaved-model", "base_url": "http://127.0.0.1:9/v1", "api_key": ""})
