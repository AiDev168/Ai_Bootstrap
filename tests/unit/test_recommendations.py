...
def test_duplicate_removal_in_consolidated_list():
    check1 = AuditCheck(name="Typer", status=CheckStatus.FAILED, category=CheckCategory.DEPENDENCIES)
    check2 = AuditCheck(name="Rich", status=CheckStatus.FAILED, category=CheckCategory.DEPENDENCIES)
