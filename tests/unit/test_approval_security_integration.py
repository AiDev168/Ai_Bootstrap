...
def test_pending_blocks_execution_completely(mock_planner, mock_validator, mock_executor, mock_audit):
...
@patch('ai_engineering_bootstrap.pipeline.engine.ExecutionPlanValidator')
@patch('ai_engineering_bootstrap.pipeline.engine.PlannerEngine')
def test_approved_proceeds_to_execution(mock_planner, mock_validator, mock_executor, mock_audit):
...
def test_replay_attack_prevention():
    ...
    req_old = provider.request_approval("act-delete-db", "plan-old", "run-100", "Old run", "CRITICAL")
    ...
    assert is_valid_for_new_run is False, "باید تشخیص دهد که این تأییدیه متعلق به ران دیگری است!"
...
def test_terminal_state_immutability():
    ...
    req_good = provider.request_approval("act-safe", "plan-1", "run-1", "Safe action", "LOW")
    ...
    req_bad = provider.request_approval("act-danger", "plan-1", "run-1", "Dangerous action", "HIGH")
...
if __name__ == "__main__":
    print("لطفاً این فایل را با دستور 'pytest test_approval_security_2.py -v' اجرا کنید.")
