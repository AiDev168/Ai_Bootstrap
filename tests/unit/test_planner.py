...
def test_planner_multiple_failures_unique() -> None:
    ...
    checks = [
        AuditCheck(name="Git", status=CheckStatus.FAILED, category=CheckCategory.TOOLS),
        AuditCheck(name="Virtual Environment", status=CheckStatus.FAILED, category=CheckCategory.ENVIRONMENT),
    ...
    assert plan.actions[0].priority < plan.actions[1].priority < plan.actions[2].priority
