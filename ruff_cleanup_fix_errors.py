...
text = p.read_text(encoding='utf-8')
text = text.replace('                with urllib.request.urlopen(request, timeout=300) as response:\n                    with open(deb_path, "wb") as f:', '                with urllib.request.urlopen(request, timeout=300) as response, open(deb_path, "wb") as f:')
p.write_text(text, encoding='utf-8')

# Session store nested condition.
p = ROOT / 'src/ai_engineering_bootstrap/environment/session_store.py'
text = p.read_text(encoding='utf-8')
text = text.replace('            if session:\n                if status_filter is None or session.status == status_filter:', '            if session and (status_filter is None or session.status == status_filter):')
p.write_text(text, encoding='utf-8')
